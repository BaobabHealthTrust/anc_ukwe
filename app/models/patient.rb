...
EOF
  ...
  def self.active_range(patient_id, date = Date.today)
    ...
    abortion_check_encounter = patient.encounters.active.find(:first, :order => ["encounter_datetime DESC"], :conditions => ["encounter_type = ? AND encounter_datetime > ? AND DATE(encounter_datetime) <= ?", EncounterType.find_by_name("PREGNANCY STATUS").encounter_type_id, date.to_date - 7.months, date.to_date]) rescue nil
    
    aborted = abortion_check_encounter.observations.collect{|ob| ob.answer_string.downcase if ob.concept_id == ConceptName.find_by_name("PREGNANCY ABORTED").concept_id}.compact.include?("yes")  rescue false 
  	
  	date_aborted = abortion_check_encounter.observations.find_by_concept_id(ConceptName.find_by_name("DATE").concept_id).answer_string rescue nil
